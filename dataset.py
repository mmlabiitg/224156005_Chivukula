...
class ToTensor(object):
    def __call__(self, sample):
        hazy_image, clean_image, depth_image = sample['hazy'], sample['clean'], sample['depth']
        ...
        depth_image = torch.transpose(torch.transpose(depth_image, 2, 0), 1, 2)
...
class Dataset_Load(Dataset):
    def __init__(self, data_path, transform=None):
        ...
        self.filesA, self.filesB, self.filesC = self.get_file_paths(self.data_path, 'UIEB')
        self.len = min(len(self.filesA), len(self.filesB), len(self.filesC))
    ...
    def __getitem__(self, index):
        ...
        depth_im = cv2.resize(cv2.imread(self.filesC[index % self.len]), (256,256),
                                  interpolation=cv2.INTER_AREA)

        depth_im = depth_im[:, :, ::-1] ## BGR to RGB   
        depth_im = np.float32(depth_im) / 255.0
    ...
    def get_file_paths(self, root, dataset_name):
        if dataset_name=='EUVP':
            ...
            for sd in sub_dirs:
                ...
                filesC = sorted(glob.glob(os.path.join(root, 'depths') + "/*.*"))
        return filesA, filesB, filesC
